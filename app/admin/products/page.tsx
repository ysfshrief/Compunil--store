...
const EMPTY_FORM = {
  ...
  brand: '', stock: '', featured: false, onSale: false,
  specs: [{ key: '', value: '' }],
};
...
export default function AdminProductsPage() {
  ...
  const openEdit = (p: Product) => {
    ...
    setForm({
      ...
      specs: p.specs?.length ? p.specs.map(s => ({ key: s.key, value: String(s.value) })) : [{ key: '', value: '' }],
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        await createProduct({ ...payload, rating: 0, reviewCount: 0, createdAt: new Date() });
                  ...
                  <div className="space-y-2">
                    {form.specs.map((spec, i) => (
                      <div key={i} className="flex gap-2">
                        ...
                        />
                        {form.specs.length > 1 && (
                          <button onClick={() => setForm(f => ({ ...f, specs: f.specs.filter((_, j) => j !== i) }))} className="text-gray-400 hover:text-red-500">
